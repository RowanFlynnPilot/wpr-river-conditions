...
- [x] **U.S. Drought Monitor county chip** — `fetch_drought()` queries all eight counties weekly. **Gotcha:** `COUNTY_NAMES` keys are SAME codes (`055073`) but the USDM `aoi` wants plain 5-digit FIPS (`55073`) — the leading zero silently returns `[]`. Counties under 1% of area are dropped as noise; the chip names only counties at the *worst* class (`worst_counties`) so a D0 county is never implied to be in D1. Attribution (NDMC/USDA/NOAA) is in the footer and required.
